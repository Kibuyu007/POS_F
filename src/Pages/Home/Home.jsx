...
const ProfitModal = ({ isOpen, onClose, profitData, totalProfit, dateRange }) => {
      ...
      >
        <div className="bg-gradient-to-r from-green-500 to-emerald-600 p-4 sm:p-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg sm:text-xl font-bold text-white">Profit Summary</h2>
              <p className="text-green-100 text-xs sm:text-sm mt-1">{dateRange}</p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="mt-4 bg-white/20 rounded-xl p-3 sm:p-4 backdrop-blur-sm">
            <p className="text-green-100 text-xs sm:text-sm font-medium">Total Net Profit</p>
            <p className={`text-2xl sm:text-3xl font-bold ${totalProfit >= 0 ? 'text-white' : 'text-red-200'}`}>
        ...
        <div className="max-h-[60vh] sm:max-h-[65vh] overflow-y-auto">
          {profitData.length === 0 ? (
            <div className="p-8 sm:p-12 text-center">
              <p className="text-gray-500 text-sm">No profit data available for this period</p>
            </div>
          ) : (
            <div className="divide-y divide-gray-100">
              <div className="grid grid-cols-12 gap-2 px-4 sm:px-6 py-3 bg-gray-50 text-xs font-semibold text-gray-600 uppercase tracking-wider">
                ...
                <div className="col-span-2 sm:col-span-2 text-right">Profit</div>
              </div>
                ...
                >
                  ...
                  <div className="col-span-6 sm:col-span-7">
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      Buy: Tsh {item.buyingPrice.toLocaleString()} → Sell: Tsh {item.sellingPrice.toLocaleString()}
                    </p>
                  </div>
        ...
        <div className="border-t border-gray-200 p-4 sm:p-6 bg-gray-50">
          <div className="flex items-center justify-between">
            <p className="text-xs text-gray-500">{profitData.length} items sold</p>
            <button
              onClick={onClose}
...
const Home = () => {
  ...
  const getDateRange = () => {
    ...
    switch (filter) {
      ...
      case "yesterday": return { from: now.subtract(1, "day").startOf("day"), to: now.subtract(1, "day").endOf("day") };
      case "week": return { from: now.startOf("week"), to: now.endOf("week") };
      case "month": return { from: now.startOf("month"), to: now.endOf("month") };
      case "custom": return { from: dayjs(range.from).startOf("day"), to: dayjs(range.to).endOf("day") };
      default: return { from: now.startOf("day"), to: now.endOf("day") };
    }
  };
    ...
    const fetchAll = async () => {
      ...
      try {
        const [txRes, expRes, grnPoRes, grnNonPoRes] = await Promise.all([
          axios.get(`${BASE_URL}/api/transactions/all`, { withCredentials: true }),
        ...
        setGrns([...(grnPoRes.data.data || []), ...(grnNonPoRes.data.data || [])]);
      } catch (err) {
  ...
  const filteredSales = useMemo(() => {
    return transactions.filter((tx) => dayjs(tx.createdAt).isBetween(from, to, null, "[]"));
  }, [transactions, from, to]);

  const filteredExpenses = useMemo(() => {
    return expenses.filter((e) => dayjs(e.createdAt).isBetween(from, to, null, "[]"));
  }, [expenses, from, to]);

  const filteredGrns = useMemo(() => {
    return grns.filter((g) => dayjs(g.receivingDate).isBetween(from, to, null, "[]"));
  }, [grns, from, to]);

  const totalSales = useMemo(() => filteredSales.reduce((s, t) => s + (t.totalAmount || 0), 0), [filteredSales]);
  const totalPaid = useMemo(() => filteredSales.filter((t) => t.status === "Paid").reduce((s, t) => s + (t.totalAmount || 0), 0), [filteredSales]);
  const totalBills = useMemo(() => filteredSales.filter((t) => t.status === "Bill").reduce((s, t) => s + (t.totalAmount || 0), 0), [filteredSales]);
  const totalDiscount = useMemo(() => filteredSales.reduce((s, t) => s + (t.tradeDiscount || 0), 0), [filteredSales]);
  
  const totalBuyingPrice = useMemo(() => {
    return filteredSales.reduce((sum, tx) => {
      const txBuying = tx.items.reduce((s, i) => s + (i.quantity || 0) * (i.buyingPrice || 0), 0);
      return sum + txBuying;
    }, 0);
  }, [filteredSales]);
  ...
  const totalGrn = useMemo(() => {
    return filteredGrns.flatMap((g) => g.items || []).reduce((s, i) => s + (i.totalCost || 0), 0);
  }, [filteredGrns]);

  const profit = useMemo(() => totalSales - (totalBuyingPrice + totalExpenses), [totalSales, totalBuyingPrice, totalExpenses]);
  ...
  const profitSummaryData = useMemo(() => {
    ...
    filteredSales.forEach((tx) => {
      ...
      tx.items.forEach((soldItem) => {
        ...
        if (!itemMap[itemName]) {
          itemMap[itemName] = { itemName, qty: 0, sellingPrice, buyingPrice, profit: 0 };
        }
        ...
        <div className="w-full lg:flex-[3] bg-secondary rounded-xl p-3 sm:p-4 lg:p-6 overflow-auto">
          ...
          <div className="mb-4 sm:mb-8">
            <div className="mb-3 sm:mb-4">
              <h2 className="text-lg sm:text-xl font-semibold text-gray-800 mb-1">Dashboard Analytics</h2>
              <p className="text-xs sm:text-sm text-gray-500">Select time period to filter data</p>
            </div>

            <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
              <div className="flex flex-wrap gap-2">
                {[
                  { id: "today", label: "Today", icon: <MdOutlineToday size={16} />, gradient: "from-emerald-500 to-teal-400" },
                  { id: "yesterday", label: "Yesterday", icon: <MdOutlineCalendarViewDay size={16} />, gradient: "from-blue-500 to-cyan-400" },
                  { id: "week", label: "Week", icon: <TbCalendarWeek size={16} />, gradient: "from-purple-500 to-pink-400" },
                  { id: "month", label: "Month", icon: <TbCalendarMonth size={16} />, gradient: "from-orange-500 to-amber-400" },
                ].map((item) => (
                  <button
                    key={item.id}
                    onClick={() => { setFilter(item.id); setRange({ from: null, to: null }); }}
                ...
                <div className="flex items-center gap-2 bg-gray-50 border border-gray-300 rounded-full px-2 sm:px-3 py-1.5 sm:py-2">
                  ...
                  <div className="flex items-center gap-1 sm:gap-2">
                    <input
                      type="date"
                      value={range.from || ""}
                      onChange={(e) => handleRangeSelect(e.target.value, range.to || e.target.value)}
                    ...
                    <input
                      type="date"
                      value={range.to || ""}
                      onChange={(e) => handleRangeSelect(range.from || e.target.value, e.target.value)}
                      className="w-28 sm:w-36 bg-transparent focus:outline-none text-xs sm:text-sm"
                    />
                  </div>
                </div>
            ...
            {filter === "custom" && range.from && range.to && (
              <div className="mb-3 sm:mb-4 p-3 sm:p-4 bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-xl">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div>
                    <p className="text-xs sm:text-sm font-medium text-gray-600 mb-0.5">Showing data for:</p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3 mt-3 sm:mt-4">
            <CardOne title="Sales" icon="💰" number={`${totalSales.toLocaleString()} TSh`} loading={loading} />
            <CardOne title="Paid" icon="✅" number={`${totalPaid.toLocaleString()} TSh`} loading={loading} />
            <CardOne title="Bills" icon="📝" number={`${totalBills.toLocaleString()} TSh`} loading={loading} />
            <CardOne title="GRN" icon="📦" number={`${totalGrn.toLocaleString()} TSh`} loading={loading} />
            <CardOne title="Expenses" icon="💸" number={`${totalExpenses.toLocaleString()} TSh`} loading={loading} />
            <CardOne title="Discounts" icon="🏷️" number={`${totalDiscount.toLocaleString()} TSh`} loading={loading} />
            <CardOne title="Buying" icon="🏭" number={`${totalBuyingPrice.toLocaleString()} TSh`} loading={loading} />
            <CardOne 
              title="Profit" 
              icon="📈" 
